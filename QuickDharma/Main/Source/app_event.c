...
PUBLIC void vEvent_RegistTask() {
	...
	// タスク登録：入力バッファ更新
	iEntryScheduleEvt(E_EVENT_UPD_BUFFER, 200, 660, TRUE);
	// タスク登録：センサーチェック処理
	iEntryScheduleEvt(E_EVENT_SENSOR_CHK, 200, 700, TRUE);
	// タスク登録：秒間隔処理
	iEntryScheduleEvt(E_EVENT_SECOND, 1000, 1500, TRUE);
}
...
PUBLIC void vEvent_SensorCheck(uint32 u32EvtTimeMs) {
	...
	// ステータスチェックの要否を判定
	if (sAppStsInfo.eAppStatus == E_APP_STS_INITIALIZE) {
		return;
	}
...
#ifdef DEBUG
			vfPrintf(&sSerStream, "MS:%08d IR Sensor Error!!!\n", u32TickCount_ms);
			SERIAL_vFlush(sSerStream.u8Device);
#endif
			return;
		}
	...
	if (bAccelerating()) {
#ifdef DEBUG
		// 加速度検知中なので後続チェックをキャンセル
		vfPrintf(&sSerStream, "MS:%08d bAccelerating Error!!!\n", u32TickCount_ms);
		SERIAL_vFlush(sSerStream.u8Device);
#endif
		return;
	}
	...
	// ドップラーセンサーチェック
	//==========================================================================
	if (sAppEventMap.eEvtDopplerSensor != 0x00) {
		if ((sAppIO.u32DiMapBef & PIN_MAP_DOPPLER_SENS) != 0) {
			// イベント処理
			sAppEventInfo.u8LogMsgCd = E_MSG_CD_DOPPLER_SENS_ERR;
			sAppEventInfo.u8UpdStsMap = APP_STS_MAP_DOPPLER_SENS;
			iEntrySeqEvt(sAppEventMap.eEvtDopplerSensor);
#ifdef DEBUG
			vfPrintf(&sSerStream, "MS:%08d Doppler Sensor Error!!!\n", u32TickCount_ms);
			SERIAL_vFlush(sSerStream.u8Device);
#endif
			return;
		}
	...
	// カラーセンサーチェック
	//==========================================================================
	if (sAppEventMap.eEvtColorSensor != 0x00) {
		if (bColorChange()) {
			// イベント処理
			sAppEventInfo.u8LogMsgCd = E_MSG_CD_COLOR_SENS_ERR;
			sAppEventInfo.u8UpdStsMap = APP_STS_MAP_COLOR_SENS;
			iEntrySeqEvt(sAppEventMap.eEvtColorSensor);
#ifdef DEBUG
			vfPrintf(&sSerStream, "MS:%08d bColorChange Error!!!\n", u32TickCount_ms);
			SERIAL_vFlush(sSerStream.u8Device);
#endif
			return;
		}
 ...
 *
 * DESCRIPTION:通常開錠状態（未警戒状態）への移行処理
 *
 * PARAMETERS:      Name            RW  Usage
 *      uint32      u32EvtTimeMs    R   イベント発生時刻
 *
...
PUBLIC void vEvent_StsUnlock(uint32 u32EvtTimeMs) {
	...
	// アプリケーションイベントマップ更新
	//==========================================================================
	sAppEventMap.eEvtDopplerSensor = 0x00;						// ドップラーセンサーイベント
	sAppEventMap.eEvtIRSensor      = 0x00;						// IR人感センサーイベント
	sAppEventMap.eEvtColorSensor   = 0x00;						// カラーセンサーイベント
 ...
 * NAME: vEvent_StsLock
 *
 * DESCRIPTION:通常施錠状態（光学センサー検知）への移行処理
 *
 * PARAMETERS:      Name            RW  Usage
 *      uint32      u32EvtTimeMs    R   イベント発生時刻
 *
...
PUBLIC void vEvent_StsLock(uint32 u32EvtTimeMs) {
	...
	//==========================================================================
	sAppEventMap.eEvtDopplerSensor = 0x00;						// ドップラーセンサーイベント
	sAppEventMap.eEvtIRSensor      = E_EVENT_STS_ALARM_LOCK;	// IR人感センサーイベント
	sAppEventMap.eEvtColorSensor   = E_EVENT_STS_ALARM_LOCK;	// カラーセンサーイベント
 ...
 *
 * DESCRIPTION:警戒施錠状態（フルセンサー検知）への移行処理
 *
 * PARAMETERS:      Name            RW  Usage
 *      uint32      u32EvtTimeMs    R   イベント発生時刻
 *
...
PUBLIC void vEvent_StsInCaution(uint32 u32EvtTimeMs) {
	...
	// アプリケーションイベントマップ更新
	//==========================================================================
	sAppEventMap.eEvtDopplerSensor = E_EVENT_STS_ALARM_LOCK;	// ドップラーセンサーイベント
	sAppEventMap.eEvtIRSensor      = E_EVENT_STS_ALARM_LOCK;	// IR人感センサーイベント
	sAppEventMap.eEvtColorSensor   = E_EVENT_STS_ALARM_LOCK;	// カラーセンサーイベント
	sAppEventMap.eEvtUnlockReq     = E_EVENT_STS_UNLOCK;		// 通常開錠リクエスト
	sAppEventMap.eEvtLockReq       = E_EVENT_STS_LOCK;			// 通常施錠リクエスト
...
PUBLIC void vEvent_StsAlarmUnlock(uint32 u32EvtTimeMs) {
	// ステータス判定
	if (sAppStsInfo.eAppStatus == E_APP_STS_ALARM_UNLOCK) {
		// 既にステータス移行済み
		return;
	}
	//==========================================================================
...
PUBLIC void vEvent_StsAlarmLock(uint32 u32EvtTimeMs) {
	// ステータス判定
	if (sAppStsInfo.eAppStatus == E_APP_STS_ALARM_LOCK) {
		// 既にステータス移行済み
		return;
	}
	//==========================================================================
