...
typedef enum {
	...
	APP_STS_MAP_IR_SENS      = (0x10),			// IR人感センサー
	APP_STS_MAP_DOPPLER_SENS = (0x20),			// ドップラーセンサー
	APP_STS_MAP_COLOR_SENS   = (0x40),			// カラーセンサー
...
typedef enum {
	...
	E_APP_CMD_LOCK       = (0x02),				// 送受信コマンド：施錠要求（光学センサーモード）
	E_APP_CMD_ALERT      = (0x03),				// 送受信コマンド：警戒施錠要求（フルセンサーモード）
...
typedef enum {
	...
	E_MSG_CD_DOPPLER_SENS_ERR,					// ドップラーセンサーエラー
	E_MSG_CD_COLOR_SENS_ERR,					// カラーセンサーエラー
...
// 構造体：アプリケーションイベントマップ
typedef struct {
	teAppEvent eEvtIRSensor;					// IR人感センサー反応時
	teAppEvent eEvtDopplerSensor;				// ドップラーセンサー反応時
	teAppEvent eEvtColorSensor;					// カラーセンサー反応時
